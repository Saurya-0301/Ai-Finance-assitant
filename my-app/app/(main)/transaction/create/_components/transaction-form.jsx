...
const AddTransactionForm=({accounts,categories,editMode=false,initialData=null}) => {
   ...
   const handleScanComplete = (scannedData) => {
    ...
    return(
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}> 
            {/* ai recipt scanner to add transaction */}
    ...
    <SelectItem value="EXPENSE">Dark</SelectItem>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                {...register("amount")} />
                {errors.amount && <p className="text-sm text-red-500">{errors.type.message}</p>}
                </div>
  ...
  <SelectContent>
    ...
    <CreateAccountDrawer>
        ...
        classname="W-full select-none items-center text -sm outline-none"> Create Account</Button>
    </CreateAccountDrawer>
        ...
        <SelectItem key={category.id} value={category.id}>
            {category.name}
             ...
             <div className="space-y-2">
                ...
                <Popover>
                 <PopoverTrigger asChild>
                    <Button variant='outline' className="w-full pl-3 text-left font-normal"> 
                        ...
                        <calendarIcon className="h1-auto h-4 w-4 opacity-50"/>
                    </Button>
                 </PopoverTrigger>
                ...
                {errors.date && <p className="text-sm text-red-500">{errors.type.message}</p>}
            </div>
            <div className="space-y-2">
                <label className="text -sm font-medium">Description</label>
                <Input placeholder="enter description" {...register("description")} />
                     {errors.description && <p className="text-sm text-red-500">{errors.type.message}</p>}
            ...
            {isRecurring && (  <div className="space-y-2">
                ...
                <Select onValueChange={(value)=>setValue("recuringInterval",value)}
                defaultValues={getValues("recuringInterval")} >
  <SelectTrigger className="w-[180px]">
